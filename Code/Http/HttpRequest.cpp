...
bool HttpRequest::parse(Buffer& buff) {
    ...
    while(buff.ReadableBytes() && state_ != FINISH) {
        // 获取一行数据，根据\r\n为结束标志
        const char* lineEnd = search(buff.Peek(), buff.BeginWriteConst(), CRLF, CRLF + 2);//在buff.Peek()到buff.BeginWriteConst()间寻找首次出现/r/n的地方，返回指针
...
bool HttpRequest::ParseRequestLine_(const string& line) {
    // GET / HTTP/1.1
    regex patten("^([^ ]*) ([^ ]*) HTTP/([^ ]*)$");//正则表达式匹配，括号代表不同组的数据
    smatch subMatch;//匹配到的数据会保存在这里面，每组数据依次放在subMatch[1]、subMatch[2]...
...
void HttpRequest::ParsePost_() {
    if(method_ == "POST" && header_["Content-Type"] == "application/x-www-form-urlencoded") {//表单提交
        // 解析表单信息
...
void HttpRequest::ParseFromUrlencoded_() {
    ...
    for(; i < n; i++) {
        ...
        switch (ch) {
        case '=':
            key = body_.substr(j, i - j);//username      =hello&password=hello
            j = i + 1;
            break;
        case '+':
            body_[i] = ' ';
        ...
        case '%':           
            ...
            break;
        case '&':
            value = body_.substr(j, i - j);//username=     hello    &password=hello
            j = i + 1;
            post_[key] = value;//键值对加入post_表单
